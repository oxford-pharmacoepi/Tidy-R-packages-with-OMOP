...
| Package | Purpose |
|---|---|
| `omopgenerics` | Defines the central classes (`cdm_reference`, `cohort_table`, `summarised_result`, codelists) and their generic methods. Every package in the ecosystem imports this. |
...
| `DrugExposureDiagnostics` | Diagnostics for drug exposure records and drug codelists. |
...
```
                        ┌─────────────────┐
                        │  omopgenerics   │  ← core classes & methods
                        └───────┬─────────┘
               ┌────────────────┼─────────────────┐
               ▼                ▼                  ▼
      ┌──────────────┐  ┌──────────────┐  ┌──────────────────┐
      │ PatientProf. │  │CodelistGen.  │  │  visOmopResults  │
      └──────┬───────┘  └──────┬───────┘  └──────────────────┘
             └──────────┬──────┘
                        ▼
            ┌──────────────────────┐
            │  Analytics packages  │  (CohortConstructor, IncidencePrevalence,
            │  Diagnostics packages│   DrugUtilisation, CohortCharacteristics,
            └──────────────────────┘   CohortSurvival, PhenotypeR, ...)
```
...
The ecosystem is under active development with frequent CRAN releases. As of early 2026 there are around 22 packages in the ecosystem, 19 of which are on CRAN, with a combined total of over 290 CRAN releases and nearly 400,000 total CRAN downloads.
